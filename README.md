...
ムードによって変わるのは首の振幅と速さだけではありません。

- **顔つき**: 眉（`Brows`）と口角（`MouthSmile`）をムードごとの値で送ります。
  どちらも 0.5 が中立で、下げると眉が寄り口角が下がります。表情ファイルとは
  別のパラメータなので併用できます。
- **仕草**: うなずき・首振り・首かしげ・前のめり・うなだれを単発で挟みます。
  ムードごとに出る仕草と頻度が違い、感情が変わった直後にも反応として出ます。
- **視線**: 画面チラ見／きょろきょろ／ガン見／二度見／視線が泳ぐ／伏し目／
  カメラ目線の7パターンをムードごとの重みで選びます。加えて、一点を見ている
  間も眼球が細かく飛びます（マイクロサッカード）。
- **波形**: 首の揺れの周期を軸ごと・ムードごとに変えているので、同じ動きの
  速度違いには見えません。

### 感情の強度を連続値で送る（任意）

モデルが `Paramtense` のような感情専用の Live2D パラメータを持っている場合、
表情ファイルの ON/OFF ではなく 0〜1 の強度で表情を出せます。ムードの補間が
そのまま表情の濃さになるので、切り替わりが滑らかになり「少し真剣」のような
中間も出せます。

起動時に `MoodTense` / `MoodSad` / `MoodExcited` / `MoodSurprised` という入力
パラメータを自動生成するので、`VTS Parameter Setup` で対応する Live2D
パラメータの input へ割り当ててください。割り当て済みかは起動時に自動判定し、
判定できたムードだけ強度方式に切り替えます（未割り当てのムードは従来どおり
表情ファイルを使うので、途中まで割り当てた状態でも壊れません）。
