...
- `quick`: 8～35文字の短い感想・ツッコミ。
- `extended`: 重大な展開だけ、最大2文・約90文字のしっかりした感想。

迷った場合は `silent` を選びます。モードと本文に矛盾する出力や文字数超過、
説明口調・不自然な体言止めは、音声化する前に最大2回まで自動で書き直します。
文字送りが細かいことを前提に、通常場面は70～80%程度を `silent` にするよう判断します。
さらに `quick` を話したあとの2ターンは次の `quick` を自動的に `silent` へ変更します。
突然の `reaction` と重大な `extended` はこの発話間隔の制限を受けません。
発話する場合は `emotion`、`intensity`、`pace` も自動選択し、次の音声応答で
演技付きで読み上げます。口調は友達と一緒に遊ぶ若い成人のようなカジュアルさを基本にし、
「へぇ」「あ、なるほど」「〜なんだよね〜」「〜じゃん」などを場面に応じて使います。
